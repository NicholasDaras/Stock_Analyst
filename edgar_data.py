...
def get_insider_trading(ticker: str, months: int = 12) -> dict:
    ...
    try:
        ...
        # Process recent filings (limit to avoid slow loads)
...
def get_institutional_holders(ticker: str) -> dict:
    """
    Fetch recent 13F institutional holding data for context.
    Returns top holders and recent changes.
    """
    try:
        _init_edgar()
        from edgar import Company
        ...
        for filing in filings[:10]:
            try:
                holders.append({
                    ...
                    "filer": str(filing.company) if hasattr(filing, 'company') else "",
                    "form": str(filing.form_type) if hasattr(filing, 'form_type') else "13G",
                })
            except Exception:
                continue
